...
{
	struct iot_easysetup_payload *request = (struct iot_easysetup_payload *)param;
	iot_error_t err;

	if (request == NULL) {
		IOT_ERROR("request is NULL");
		return;
	}
	...
	if (err != IOT_ERROR_NONE) {
		 ...
		 */
		if (request->step == IOT_EASYSETUP_STEP_SETUPCOMPLETE) {
				...
				enum iot_easysetup_step step, const void *payload)
{
	...
	request = iot_os_malloc(sizeof(struct iot_easysetup_payload));
	if (request == NULL)
	{
		IOT_ERROR("Failed to malloc for easyetup request");
		return IOT_ERROR_MEM_ALLOC;
	}
	memset(request, 0, sizeof(struct iot_easysetup_payload));
	...
	err = iot_put_device_work(ctx, _easysetup_request_work_handler, (device_work_param)request);
	if (err != IOT_ERROR_NONE) {
		iot_os_free(request);
	}
...
iot_error_t iot_misc_info_store(iot_misc_info_t type, const void *in_data)
{
	...
	if (new_misc_info == NULL) {
		IOT_ERROR("Fail to make new_misc_info string");
		iot_err = IOT_ERROR_BAD_REQ;
		goto misc_info_store_out;
	}
